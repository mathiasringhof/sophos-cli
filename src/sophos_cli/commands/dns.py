...
def _render_entries(entries: list[DnsHostEntryCreate], output: OutputFormat) -> None:
    if resolve_output_format(output) == "json":
        render_payload([entry.model_dump() for entry in entries])
        return
...
def dns_list(
    ...
    output: Annotated[
        OutputFormat,
        typer.Option("--output", help="Response format: auto, table, or json."),
    ] = "auto",
...
@dns_app.command("add", deprecated=True, hidden=True)
def dns_add(
    ctx: typer.Context,
    host_name: Annotated[str, typer.Argument(help="DNS host entry name (hostname or FQDN).")],
    ip_address: Annotated[
    ...
    """Deprecated alias for `dns create`."""
...
@dns_app.command(
    "create-many",
    epilog="Use `dns add-many` only for backward compatibility.",
)
def dns_create_many(
    ctx: typer.Context,
    file_path: Annotated[
    ...
    _render_bulk_summary("create-many", result)
    if result.failed > 0:
        raise typer.Exit(code=2)


@dns_app.command("add-many", deprecated=True, hidden=True)
def dns_add_many(
    ctx: typer.Context,
    file_path: Annotated[
        str,
    ...
    """Deprecated alias for `dns create-many`."""
...
def dns_delete(
    ...
    yes: Annotated[
        bool,
        typer.Option("--yes", help="Confirm deletion of the DNS host entry."),
    ] = False,
    host: HostOption = None,
    username: UsernameOption = None,
    password: PasswordOption = None,
    ...
    try:
        service = _build_service(ctx, host, username, password, port, insecure)
        response = service.delete_entry(host_name)
    except API_EXCEPTIONS as exc:
        handle_api_exception(exc)

    console.print(f"DNS entry '{host_name}' deleted")
    render_payload(response)
